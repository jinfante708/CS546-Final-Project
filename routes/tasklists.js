...
router.get("/upcoming", async (req, res) => {
    ...
    try {
        ...
        res.status(500).render("tasklists/upcoming", {
            pageTitle: "Upcoming tasks",
            error: e,
        });
    }
...
router.get("/:id", async (req, res) => {
    // this route is actually not in use
...
router.put("/:id", async (req, res) => {
    // this is actuallhy not in use too because we don't have a "edit task list" feature
    ...
    if (!listInfo) {
        res.status(400).json({
            error: "you must provide data to update a task list.",
        });
        return;
    }
