...
let exportedMethods = {
  async gettasklistid(taskid) {
    ...
    alltasklist.forEach((element) => {
      for (i = 0; i < element.tasks.length; i++) {
        if (element.tasks[i] == taskid) a = element._id;
      }
    });
    return a;
  ...
  async create(name, importance, deadlineDate, userId) {
    ...
    let priorityformula = Math.sqrt(
      Math.pow(240 / urgencyformula, 2) + Math.pow(importance, 2)
    );
